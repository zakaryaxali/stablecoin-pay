...
import { ApyRate, formatApy, formatTvl, getPoolName } from "@/services/apy";
...
export function APYTable({ rates, bestPoolId, onDeposit }: APYTableProps) {
  if (rates.length === 0) {
    return (
  ...
  return (
    <View className="bg-white rounded-2xl mx-4 mt-4 overflow-hidden">
      ...
      {rates.map((rate, index) => {
        const poolKey = rate.pool_id ?? rate.platform;
        const isBest = poolKey === bestPoolId;
            ...
            <View className="flex-1">
              <View className="flex-row items-center">
                <Text className="text-gray-900 font-semibold">
                  {getPoolName(rate.pool_meta, rate.platform)}
                </Text>
                {isBest && (
